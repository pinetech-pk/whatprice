  ...
  const handleCreateVendor = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/admin/vendors', {
        ...
        body: JSON.stringify({
          ...createForm,
          viewCredits: parseInt(createForm.viewCredits) || 100,
        }),
      });
      const data = await res.json();
      if (!res.ok) {
      ...
      alert(`Vendor "${data.vendor.storeName}" created successfully!`);
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-2xl w-full p-6 my-8">
            ...
            <form onSubmit={handleCreateVendor} className="space-y-6">
              ...
              {/* Store Information */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-3">Store Information</h4>
                <div className="grid grid-cols-2 gap-4">
              ...
              {/* Admin Settings */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-3">Admin Settings</h4>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Verification Status</label>
                    <select
                      value={createForm.verificationStatus}
                      onChange={(e) => setCreateForm({ ...createForm, verificationStatus: e.target.value })}
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Initial Credits</label>
                    <input
                      type="number"
                      min="0"
                      value={createForm.viewCredits}
                      onChange={(e) => setCreateForm({ ...createForm, viewCredits: e.target.value })}
                      className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                </div>
